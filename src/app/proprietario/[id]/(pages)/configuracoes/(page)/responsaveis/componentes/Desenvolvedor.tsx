...
export default function Desenvolvedor({ proprietarioId }: { proprietarioId?: string }) {
  ...
  const [currentDesenvolvedor, setCurrentDesenvolvedor] = useState<Partial<Responsavel>>(() => ({
  ...
  const [selectedDesenvolvedorDetails, setSelectedDesenvolvedorDetails] = useState<Responsavel | null>(null);
  ...
  const openModal = (desenvolvedor?: Responsavel) => {
    if (desenvolvedor) {
  ...
  const showDesenvolvedorDetails = (desenvolvedor: Responsavel) => {
            ...
            <div className="bg-white rounded-lg shadow-2xl p-6 w-full max-w-md">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800">
                  {isEditMode ? 'Editar Responsavel' : 'Novo Responsavel'}
                </h2>
                <button 
